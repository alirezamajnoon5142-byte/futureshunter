...
def _v71_sr_candle_stats(closed, offset=-1):
...
def _v71_live_candidate_gate(result, trades):
    ...
    return {"eligible":not reasons,"decision":"ALLOW" if not reasons else "SKIP","reasons":reasons,
            ...
            "evaluated_ts":time.time(),"version":"7.8.3-breakout-aware-sr","live_exposure_count":risk.get("open_count",0),"live_exposure_symbols":risk.get("open_symbols",[]),"exposure_source":risk.get("exposure_source")}
